...
function drawEmoji(data) {
  const { context, particle, radius, opacity } = data, emojiData = particle.emojiData, double10 = 2, diameter = radius * double10, previousAlpha = context.globalAlpha;
  if (!emojiData) {
    return;
  }
...
var EmojiDrawer = class {
  ...
  particleInit(container, particle) {
    const double10 = 2, shapeData = particle.shapeData;
    ...
    const canvasSize = getRangeMax(particle.size.value) * double10;
    let emojiData;
    ...
    if (typeof OffscreenCanvas !== "undefined") {
      ...
      context.font = `400 ${maxSize * double10}px ${font}`;
  ...
  constructor(container) {
    ...
    this._clickBubble = () => {
      ...
      for (const particle of query) {
        ...
        if (timeSpent > bubbleOptions.duration * double3) {
...
var Collider = class extends ParticlesInteractorBase {
  ...
  interact(p1, delta) {
    ...
    const container = this.container, pos1 = p1.getPosition(), radius1 = p1.getRadius(), query = container.particles.quadTree.queryCircle(pos1, radius1 * double5);
    for (const p2 of query) {
...
var CircleWarp = class extends Circle {
  ...
  intersects(range) {
    ...
    if (circle.radius !== void 0) {
      const biggerCircle = new Circle(newPos.x, newPos.y, circle.radius * double6);
      return super.intersects(biggerCircle);
    } else if (rect.size !== void 0) {
      const rectSW = new Rectangle(newPos.x, newPos.y, rect.size.width * double6, rect.size.height * double6);
...
var LinkInstance = class {
  constructor(container) {
    ...
    this._drawLinkTriangle = (p1, link1, link2) => {
      ...
      const container2 = this.container, options = container2.actualOptions, p2 = link1.destination, p3 = link2.destination, opacityTriangle = triangleOptions.opacity ?? (link1.opacity + link2.opacity) * half8;
...
var RotateUpdater = class {
  ...
  init(particle) {
    ...
    if (rotateDirection === RotateDirection.random) {
      const index = Math.floor(getRandom() * double8), minIndex = 0;
...
function drawSquare(data) {
  const { context, radius } = data, fixedRadius = radius / fixFactor, fixedDiameter = fixedRadius * double9;
